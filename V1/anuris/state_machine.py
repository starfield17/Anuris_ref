...
class ChatStateMachine:
    ...
    def __init__(self, config: Config, ui: ChatUI, workspace_root: Optional[Path] = None):
        ...
        self.attachment_manager = self.session.attachment_manager
        self.agent_runner = self.session.agent_runner
        self.command_dispatcher = self.session.command_dispatcher
    ...
    def _handle_responding_state(self) -> ChatState:
        ...
        try:
            self.session.handle_input(self.context["user_input"])
            return ChatState.WAITING_FOR_USER
